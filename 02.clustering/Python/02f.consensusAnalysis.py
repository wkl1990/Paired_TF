...
# Copy from Songpeng CEMBA SnapATAC

import argparse

parser = argparse.ArgumentParser(description="leiden on 3-cols knn sparse matrix")
parser.add_argument(
    "-i", "--in", type=str, dest="input", help="input 3-col file: peak, cell, ct"
)
parser.add_argument(
    "-r", "--resolution", type=float, dest="resolution", help="resolution from 0-1"
)
parser.add_argument(
    "--u1", type=float, dest="u1", default=0.05, help="left interval cutoff of CDF"
)
parser.add_argument(
    "--u2", type=float, dest="u2", default=0.95, help="right interval cutoff of CDF"
)
parser.add_argument("-n", type=int, dest="N", help="iteration of N time")
parser.add_argument(
    "-s", "--sample", type=int, dest="sample", default=5000, help="sample # of cell"
)
parser.add_argument(
    "-o", "--out", type=str, dest="output", help="prefix of output file"
)
