...
umap_rna <- function(RNAdata, npc=20, umap="raw", assay="integrated") {
  ...
  if (umap=="raw") {
    if (ncol(RNAdata)<=30) {
      ...
      RNAdata <- RunUMAP(RNAdata, dims=1:npc, a=1.8956, b=0.8006, verbose=FALSE, n.neighbors=ceiling(ncol(RNAdata)/2), return.model=TRUE)
    } else {
      RNAdata <- RunUMAP(RNAdata, dims=1:npc, a=1.8956, b=0.8006, verbose=FALSE, return.model=TRUE)
    }
  } else if (umap=="seurat") {
    if (ncol(RNAdata)<=30) {
      print("Warning: Sample number is less than 30!")
      RNAdata <- RunUMAP(RNAdata, dims=1:npc, verbose=FALSE, n.neighbors=ceiling(ncol(RNAdata)/2), return.model=TRUE)
    } else {
      RNAdata <- RunUMAP(RNAdata, dims=1:npc, verbose=FALSE, return.model=TRUE)
    }
  } else {
