...
parser$add_argument("-d", "--dourate", default=NULL, help="pre-defined doublet rate")
...
if (!is.null(args$dourate)){
  dourate = as.numeric(args$dourate)
}
...
doublet_rna <- function(RNAdata, npc=20, filter=TRUE, min.cells=1, pre_dourate=NULL) {
  ...
  # pK Identification
  set.seed(2023)
  sweep.res.list_RNAdata <- paramSweep(RNAdata, PCs=1:npc, sct=FALSE)
  ...
  dourate <- ifelse(!is.null(pre_dourate), pre_dourate, doublet_rate[match(recoverNum, doublet_rate$recoverCells), "rate"])
  ...
  RNAdata <- doubletFinder(RNAdata, PCs=1:npc, pN=0.25, pK=pK, nExp=nExp_poi.adj, reuse.pANN=FALSE, sct=FALSE)
  # filter
  if (filter==TRUE) {
...
#user_dourate <- ifelse((exists(quote(dourate))), dourate, NULL)
if (exists(quote(dourate))) {
  user_dourate <- dourate
} else {
  user_dourate <- NULL
}
if (length(RNAdata)==1) {
  RNAdata_doubletlist <- doublet_rna(RNAdata, npc=npc, filter=dfilter, min.cells=mincells, pre_dourate=user_dourate)
  RNAsceList <- RNAdata_doubletlist
  #RNA_cells_doublet <- paste(names(RNAsceList), colnames(RNAsceList), sep=":")
  RNA_cells_doublet <- colnames(RNAsceList)
} else {
  RNAsceList <- list()
  RNA_cells_doublet <- c()
  for (i in names(RNAdata)) {
    RNAdata_doubletlist <- doublet_rna(RNAdata[[i]], npc=npc[[i]], filter=dfilter, min.cells=mincells, pre_dourate=user_dourate)
...
saveRDS(RNAsceList, file=paste0(outF, ".doublet.rds"))
write.table(RNA_cells_doublet, file=paste0(outF, ".cells_doublet.txt"), row.names=FALSE, col.names=FALSE, quote=FALSE)
print("Job is done!")
