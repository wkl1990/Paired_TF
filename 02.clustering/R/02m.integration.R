...
parser$add_argument("-n", "--normalized", default="Strandard", help="normalization method, Strandard or SCTransform or VariableFeatures or None")
...
parser$add_argument("-s", "--save_rds", action="store_true", help="save combined data")
...
save_rds = ifelse(args$save_rds, TRUE, FALSE)
...
if (normalized=="Strandard") {
  ...
  # Run the standard workflow for visualization and clustering
  se.combined <- ScaleData(se.combined, verbose = FALSE)
} else if (normalized=="VariableFeatures") {
  # Strandard normalization
  # normalize and identify variable features for each dataset independently
  se.lst <- lapply(X=se.lst, FUN=function(x) {
  #    x <- NormalizeData(x)
...
if (save_rds) {
  saveRDS(se.combined, paste0(outF, ".combined.rds"))
}
