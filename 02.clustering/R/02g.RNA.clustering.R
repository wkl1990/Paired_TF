...
parser$add_argument("-g", "--gene", default="Ctcf,Polr2a", help="gene name to plot")
...
if (uplot) {
  ...
  if (length(RNAdata)==1) {
    ...
    if (is.null(RNAsceList@meta.data[[sampleID]])) {
      RNAsceList@meta.data[[sampleID]] <- factor(RNAsceList@meta.data$orig.ident)#, levels=c("XJ554", "XJ555", "XJ556", "XJ557", "XJ558", "XJ559"))
    }
    ...
    ggsave(paste0(outF, ".", sampleID, ".split.umap.pdf"), plot=pt_splitdis, width=8, height=4 * ceiling(length(unique(RNAsceList@meta.data[[sampleID]])) / 2),limitsize = FALSE)
    ...
    if (ncol(RNAsceList)<100000) {
      pt_size <- 0.1
    } else {
      pt_size <- NULL
    }
    pt_gene <- FeaturePlot(object=RNAsceList, features=gene, pt.size=pt_size, max.cutoff="q95", reduction="umap", label=FALSE, ncol=2)
    ...
    for (i in names(RNAdata)) {
      ...
      if (ncol(RNAsceList[[i]])<100000) {
        pt_size <- 0.1
      } else {
        pt_size <- NULL
      }
      pt_gene <- FeaturePlot(object=RNAsceList[[i]], features=gene, pt.size=pt_size, max.cutoff="q95", reduction="umap", label=FALSE, ncol=3)
