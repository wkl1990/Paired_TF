"""
Copy from Songpeng
Modify Yang's 02.nmfATAC.stat.py
- Add another input mat support:
  h5 file (feature by cluster)
"""
import argparse
...
parser.add_argument('-c', '--contribute',
                    type=float, dest="contribute",
                    default=0.3,
                    help='an float cutoff for contribute of bins / cells')
...
def read_files(npz = None, h5 = None,
               xgi = None, ygi = None,
               basis = None, coef = None):
    """
    ...
    """
    if npz is not None:
        print(f"Loading mat from npz file {npz}.")
        V = load_npz(npz)
        ## to sparse row matrix
        V = V.tocsr()
    elif h5 is not None:
        print(f"Loading mat from h5 file {h5}.")
