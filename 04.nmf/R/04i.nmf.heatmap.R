...
op <- list(
  ...
  make_option(c("--downsample"), action = "store_true", default = TRUE),
  make_option(c("--lowcutoff"), type = "double", default = 0.01),
  make_option(c("--highcutoff"), type = "double", default = 0.99)
)
...
downsample = ifelse(args$downsample, TRUE, FALSE)
...
p.cpm.log2 <- getNMFHeatMap(
  ...
  ha_col = ha_col,
  fontsize = 30,
  low.val.col = quantile(logcpm, lowcutoff),
  high.val.col = quantile(logcpm, highcutoff),
  legend_labels=c(round(min(logcpm),0), round(max(logcpm),0)))

# * plot figures
figfnm <- file.path(args$figd,
    paste(tag, paste0("r", module), paste0("ds", downsample), paste0("low", lowcutoff), paste0("high", highcutoff), "pdf", sep = "."))
withr::with_pdf(
  new = figfnm,
  code = {
    print(p.cpm.log2)
...
saveRDS(
  ...
  file = file.path(args$figd,
    paste(tag, paste0("r", module), paste0("ds", downsample), paste0("low", lowcutoff), paste0("high", highcutoff), "meta.rds", sep = "."))
)
