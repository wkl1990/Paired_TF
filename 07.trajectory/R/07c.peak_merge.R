...
parser$add_argument("-e", "--extend", default = 500, help="extend to [default %(default)s] bp length")
...
ext_size = as.numeric(args$extend)
outDir = args$dir
outF = args$output
...
peak.list = lapply(seq(file.lst), function(i){
  ...
  p.gr <- extendSummit(p.gr, size=ext_size)
