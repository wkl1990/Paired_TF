...
for (i in 1:nrow(input)){
  file_name <- paste("Paired-Tag_QC_reports/QC_Summary_", as.vector(input[i,1]), "_", as.vector(input[i,2]), ".xls", sep="")
  meta[[i]] <- read.csv(file_name, sep="\t", header = T, row.names = 1)
}
meta <- do.call(cbind, meta)
write.table(meta, col.names=T, row.names=T, sep="\t", quote=F, file="Paired-Tag_QC_reports/QC_Summary_metaData_Table.xls" )
```


```{r plot, include=TRUE,echo=FALSE, fig.width=17, fig.height=8}
meta.s<-meta
