#!/usr/bin/bash
# example: bash 01b.DNA_preprocess_SE.sh -s XJ548 -g mm10 -m PT3 -p example-folder/

tscc=/tscc/projects
reachtool=${tscc}/package/Paired-Tag-master/reachtools/reachtools
rm_dup=rmdup_nh

while getopts ':s:g:m:t:l:r:p:h' opt; do
  case "$opt" in
    s)
      sampleID="$OPTARG"
...
ln -s ${fastq_dir}/${sampleID}_DNA_combined.fq.gz ${fastq_dir}/${sampleID}_combined.fq.gz
...
${reachtool} ${rm_dup} ${map_dir}/${salll}_${genome}_sorted.bam
