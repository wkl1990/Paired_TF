...
CTCF_info["path"] = bam_dir + CTCF_info["DNA"] + "_mm10_sorted_rmdup.bam"

print(all(sample in pd.unique(CTCF_info["DNA"]) for sample in meta["sampleID"]))
...
def generate_bams(CTCF_info, meta, outPrefix):
    ...
    for idx in np.unique(meta['celltype'].astype(str)):
        for sample in np.unique(CTCF_info['DNA'].astype(str)):
            bamf = CTCF_info.loc[CTCF_info["DNA"] == sample, "path"].values[0]
